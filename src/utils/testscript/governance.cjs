...
function convertHcsTopicIdToHedera(bytes32TopicId) {
  try {
    ...
    if (
      typeof bytes32TopicId === "string" &&
      bytes32TopicId.match(/^\d+\.\d+\.\d+$/)
    ) {
      console.log(
        c.dim(`     ✅ Topic ID already in Hedera format: ${bytes32TopicId}\n`)
      );
...
async function testProposalCreation(ajoGovernance, members, ajoInfo) {
  ...
  for (const p of proposals) {
    console.log(
      c.dim(
        `     │ ${p.id.toString().padStart(2)} │ ${p.description.padEnd(
          31
        )} │ ${p.proposer.padEnd(11)} │ ${p.type.padEnd(18)} │`
      )
    );
  }
  console.log(
...
async function testRealHcsVoteSubmission(
  ...
  console.log(
    c.cyan(
      `  🗳️  Submitting ${
        hederaClient ? "Real" : "Simulated"
      } Votes to HCS for Proposal #${proposalId}...\n`
    )
  );
  ...
  for (let i = 0; i < Math.min(7, members.length); i++) {
    ...
    try {
      ...
      if (hcsResult.success) {
        ...
        console.log(
          c.dim(
            `     │ ${(i + 1).toString().padStart(2)} │ ${member.name.padEnd(
              11
            )} │ ${voteText} │ ${hcsResult.sequenceNumber
              .toString()
              .padEnd(14)} │ ${txId} │ ${statusText.padEnd(20)} │`
          )
        );
      } else {
        console.log(
          c.dim(
            `     │ ${(i + 1).toString().padStart(2)} │ ${member.name.padEnd(
              11
            )} │ ${voteText} │ ${"-".padEnd(14)} │ ${"-".padEnd(11)} │ ${c
              .red("❌ Failed")
              .padEnd(20)} │`
          )
        );
...
async function testSeasonStatus(ajoGovernance) {
  ...
  try {
    ...
    console.log(
      c.dim(
        `     │ Current Season:          ${seasonStatus._currentSeason
          .toString()
          .padEnd(24)} │`
      )
    );
    console.log(
      c.dim(
        `     │ Season Completed:        ${(seasonStatus._isSeasonCompleted
          ? "Yes"
          : "No"
        ).padEnd(24)} │`
      )
    ...
    console.log(
      c.dim(
        `     │ Participation Deadline:  ${seasonStatus._participationDeadline
          .toString()
          .padEnd(24)} │`
      )
    );
    console.log(
      c.dim(
        `     │ Declared Participants:   ${seasonStatus._declaredParticipants
          .toString()
          .padEnd(24)} │`
      )
    );
    ...
    console.log(
      c.dim(
        `     │ Carry Reputation:        ${(carryOverRules._carryReputation
          ? "Yes"
          : "No"
        ).padEnd(24)} │`
      )
    ...
    console.log(
      c.dim(
        `     │ Carry Penalties:         ${(carryOverRules._carryPenalties
          ? "Yes"
          : "No"
        ).padEnd(24)} │`
      )
...
async function testProposalExecution(ajoGovernance, proposalId, executor) {
  ...
  try {
    ...
    console.log(
      c.dim(
        `     │ Votes Needed         │ ${status.votesNeeded
          .toString()
          .padEnd(7)} │`
      )
    );
    ...
    try {
      ...
      console.log(
        c.dim(
          `     │ ${(i + 1).toString().padStart(2)} │ ${member.name.padEnd(
            11
          )} │ ${participationText} │ ${c.green("✅ Declared").padEnd(20)} │`
        )
      );
      declarations.push({
        member: member.name,
      ...
      console.log(
        c.dim(
          `     │ ${(i + 1).toString().padStart(2)} │ ${member.name.padEnd(
            11
          )} │ ${participationText} │ ${c.red("❌ Failed").padEnd(20)} │`
        )
      );
    }
    await sleep(500);
